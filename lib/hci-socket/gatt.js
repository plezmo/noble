...
var globalLastWriteWithoutResponseSentAt = 0;
Gatt.prototype.delay = 10; // 5 ms 
Gatt.prototype.write = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  var characteristic = this._characteristics[serviceUuid][characteristicUuid];

  if (withoutResponse) {
    var now = new Date().getTime();
    if((now - globalLastWriteWithoutResponseSentAt) >= this.delay) {
      // We sent last write without response DELAY time ago. 
      // It is safe to send one now
      this._queueCommand(this.writeRequest(characteristic.valueHandle, data, true), null, function() {
        ...
        //console.warn("Sending write without response for " + this._address);
      }.bind(this));
      globalLastWriteWithoutResponseSentAt = now;
      
    } else {
      globalLastWriteWithoutResponseSentAt += this.delay;
      var delay = globalLastWriteWithoutResponseSentAt - now;
      var instance = this;
      window.setTimeout(function() {
        instance._queueCommand(instance.writeRequest(characteristic.valueHandle, data, true), null, function() {
          instance.emit('write', instance._address, serviceUuid, characteristicUuid);
          //console.warn("Sending write without response for " + instance._address);
        }.bind(instance));
      }, delay);
    }
